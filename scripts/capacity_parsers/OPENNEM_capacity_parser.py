...
def filter_capacity_data_by_datetime(data: pd.DataFrame, target_datetime: datetime) -> pd.DataFrame:
    df = data.copy()
...
def get_capacity_for_all_zones(target_datetime: datetime):
    ...
    capacity_df["datetime"] = capacity_df["datetime"].apply(lambda x: pd.to_datetime(x, utc=True).replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0))
